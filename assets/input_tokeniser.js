...
// Form elements should have a parent of #filter-buttons
  ...
  $.fn.liveFilter = function(targetEl) {
    // targetEl = target input element
    ...
    $('#filter-buttons input, #filter-buttons select').on('change', '', function(e){
      ...
      // Update the text box
      $(targetEl).val(liveFilterNewValue).trigger("change");
    });

    function updateIncoming() {
      var inputString = $(targetEl).val().join(' ').toLowerCase() + " " + $('.select2-results-dept-0.select2-result.select2-result-selectable.select2-highlighted').text().toLowerCase();
